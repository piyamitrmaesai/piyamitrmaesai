...
fs.writeFileSync(path.join(dist,'site-content.json'),JSON.stringify({generated_at:new Date().toISOString(),news,documents,announcements,gallery,sources,settings},null,2));
...
const pages=['','about.html','missions.html','news.html','volunteer.html','donate.html','downloads.html','contact.html'];const base=(process.env.URL||'https://YOUR-SITE.netlify.app').replace(/\/$/,'')+'/';fs.writeFileSync(path.join(dist,'sitemap.xml'),'<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n'+pages.map(p=>`  <url><loc>${base}${p}</loc></url>`).join('\n')+'\n</urlset>\n');
